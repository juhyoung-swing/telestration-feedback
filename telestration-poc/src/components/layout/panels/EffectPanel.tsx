...
import type { CircleParams, FeatureId, FreehandStroke, Mode, Overlay, PathArrow, PathParams, Players, PoseAngleId, PoseOverlay, SpeedSegment, TextLabel, TextParams, ZoneParams, ZoomParams } from '../../../types';
...
type Props = {
  ...
  selectedFreehand: FreehandStroke | null; // a selected pen stroke → editable (color/width)
  slowmoRate: number;
      ...
      {FEATURE_GROUPS.map((g) => (
        <div key={g} className="feature-group">
          ...
          <div className="feature-grid">
            {FEATURES.filter((f) => f.group === g && f.id !== 'freehand').map((f) => {
          ...
          {p.selected === 'freehand' && p.selectedFreehand && (
            <>
              <div className="field"><label>펜 색</label>
                <input type="color" value={p.selectedFreehand.color ?? '#FFD400'} onChange={(e) => p.onPatchOverlay(p.selectedFreehand!.id, { color: e.target.value })} /></div>
              <div className="field"><label>굵기 {p.selectedFreehand.width}px</label>
                <input type="range" min="1" max="16" step="1" value={p.selectedFreehand.width} onChange={(e) => p.onPatchOverlay(p.selectedFreehand!.id, { width: Number(e.target.value) })} /></div>
              <div className="muted-note">선택된 펜 선. 색·굵기를 바꾸거나 🗑로 지우세요. (새 펜은 아래 COACH 바에서)</div>
            </>
          )}
